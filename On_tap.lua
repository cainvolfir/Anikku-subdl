...
local CONFIG = {
  ...
  TMDB_SEARCH_MAX = 3 -- take top results
  -- Minimal addition: limit how many subtitles to download per imdb id.
  -- Set to 0 or nil for unlimited (default behavior).
  , MAX_PER_IMDB = 5
}
...
-- ========== Helper: add all .srt from a folder into mp (Aniyomi) ==========
local function normalize_dir(dir)
  if not dir then return dir end
  return dir:gsub("/+$", "")
end

local function add_srt_from(dir)
  if not dir or dir == "" then
    aniyomi.show_text("No folder specified")
    return
  end

  local entries = utils.readdir(dir, "files")
  if not entries then
    aniyomi.show_text("Folder not found or not readable:\n" .. dir)
    return
  end
  ...
  for _, name in ipairs(entries) do
    if name:lower():match("%.srt$") then
      local subpath = utils.join_path(dir, name)
      mp.commandv("sub-add", subpath)
      added = added + 1
    end
  end

  aniyomi.show_text("Added " .. tostring(added) .. " .srt from:\n" .. dir)
end
...
_G.downloadSubs = function()
  ...
  -- Automatically add all .srt from the download folder to Aniyomi/mp
  local dir_to_scan = normalize_dir(CONFIG.DOWNLOAD_DIR)
  -- Use add_srt_from; if the folder isn't found it will show a message.
  add_srt_from(dir_to_scan)
end
